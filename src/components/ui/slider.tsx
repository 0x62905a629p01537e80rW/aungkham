...
>(({ className, showValue = true, formatValue, ...props }, ref) => {
  ...
  return (
    <SliderPrimitive.Root
      ref={ref}
      className={cn(
        "glass-tile relative flex w-full touch-none select-none items-center rounded-full px-2 py-1.5",
        className,
      )}
      {...props}
    >
      <SliderPrimitive.Track className="relative h-1 w-full grow rounded-full bg-foreground/10">
        {/* tick dots */}
      ...
      {values.map((v, i) => (
        <SliderPrimitive.Thumb
          key={i}
          className={cn(
            "group grid h-5 w-8 place-items-center rounded-[9px]",
        ...
        >
          {showValue && (
            <span className="pointer-events-none select-none font-mono text-[8px] font-semibold leading-none tabular-nums text-foreground/70">
